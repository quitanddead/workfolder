...
/*  АДМИНИСТРАЦИЯ UID      		 */ 			_SAdmins = ["2672452"]; 								/* Пишем UID игрока для выдачи прав Администрации. */
...
/* ***************************** ESC МЕНЮ **************************************** */
/*  ESC меню Верхний текст       */ 			_TopOfESC = "TAVIANA OVERPOCH"; 				/* Сообщение которое будет показываться наверху меню ESC. */
/*  ESC меню Нижний текст #1   	 */ 			_LowerTop = "AntiHack"; 				/* Сообщение которое будет показываться внизу меню ESC. */
...
/* ***************************** ДЕБАГ МОНИТОР **************************************** */
/*  Использовать Дебаг Монитор      			*/ 			_DMS =  false;									/* true - Включить. False - Выключить. */
/*  Кнопка Активации/Деактивании Дебаг Монитора */ 			_ODK =  0xCF;									/* Не трогать!!! (0xCF - это END по умолчанию!). */
/*  Сообщение в дебаг мониторе (Внизу)      	*/ 			_BottomDebug = "TAVIANA OVERPOCH"; 		/* Сообщение которое будет в самом низу дебаг монитора. */
...
/*  Использовать проверку АнтиХака?    */ 			_AHL = true;		/* Рекомендуется:  true 	  */				/* АнтиХак проверяет отклик игрока к системе АнтиХака, если ответа в обращении игры не будет, то игрок будет кикнут. */
/*  Время отклика     			  	   */ 			_TDI =   450;		/* Рекомендуется:  45 - 300   */				/* Только при "_AHL = true;" Как долго АнтиХак будет делать задержку в отклике? */
...
/*  Восстанавливать игровые функции?   */			_FRC =  false;		/* Рекомендуется:  true 	  */				/* Игровые функции читака будут восстановлены по умолчанию. Это лишает читака некоторых возможностей. */
...
/* ********************************* Итемы ******************************************** */
/*  Удалять отспавненные вещи  		   */ 			_RAI =  false;		/* Рекомендуется:  true		  */				/* Удаляет отспавненные вещи. Если используется нестандартный крафтинг - установить False. */
...
/*  РАЗРЕШЕННЫЕ ДИАЛОГ СЦЕНАЦИИ       */ _ALLOWED_Dialogs = [-1,106,2200,6900,6901,6902,6903,571113,420420,41144,420000,420001,420002,420003,420004,420005,711194,88890];
...
/*  Блокировать все неизвестые Командные меню (CMD)     */ _BCM = false;		/* Рекомендуется:  true */
