...
_victimcharID = player getVariable ["CharacterID", "0"];
_sourcecharID = _source getVariable ["CharacterID", "0"];
_victimfriends = player getVariable ["friendlies", []];
_sourcefriends = _source getVariable ["friendlies", []];

if ((_victimcharID in _sourcefriends) && (_sourcecharID in _victimfriends)) exitWith {
	diag_log text format["DAMAGE: %1 hit from friend %2 in %3 with %4 for %5",name player,name _source,_hit,_ammo,str(_damage)];
};
